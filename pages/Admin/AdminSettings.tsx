...
import { isCloudConnected, saveToCloud, initSupabase, getSupabaseConfig, supabase, fetchFromCloud, saveSupabaseConfig } from '../../utils/supabase.ts';
...
const AdminSettings: React.FC<AdminSettingsProps> = ({ settings, setSettings, products, setProducts, categories, setCategories }) => {
  ...
  const testConnection = async () => {
    ...
    try {
      // Save config globally (localStorage + Supabase)
      await saveSupabaseConfig(cloudConfig.projectId, cloudConfig.key);

      // Re-init the bridge with the new keys
      const client = await initSupabase(true);
      ...
      setTestStatus({ type: 'success', message: '✓ Global System Linked & Config Saved Worldwide' });
...
CREATE TABLE IF NOT EXISTS dm_config (id TEXT PRIMARY KEY, config JSONB NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW());
